...
class MheSingleShooting:
    ...
    def update_data(self, t_stamp: float, data_typ: str, data):
        if self.initialized:
            if data_typ == 'imu':
                ...
                x_curr = self.constrain_yaw_angle(x_curr)               # make yaw angle in an interval
                ...
                x_curr = self.constrain_yaw_angle(x_curr)
    ...
    def update_obj(self):
        ...
        for (i, id_i) in enumerate(id_list_all[:-1]):       # loop M times
            ...
            if id_i in self.id_list_mea:
                id_j = self.id_list_mea.index(id_i)         # find the same item in 'id_list_mea' and get its id
                self.sym_xx[:, id_j] = sym_x_next           # record sym_xx, (the last one should be recorded after the loop)
                sym_x_next = self.f_plus_w(sym_x_next, param_u[:, i], ts_horizon[i], self.sym_ww[:, id_j])
            # If the current point is imu, integrate to the next point using latest w
            else:
                # id_j kept in the measurement point
                sym_x_next = self.f_plus_w(sym_x_next, param_u[:, i], ts_horizon[i], self.sym_ww[:, id_j])
        self.sym_xx[:, -1] = sym_x_next
        ...
        delta_x0 = self.sym_x0 - self.xk0
        # delta_x0[2] = ca.tan(0.5 * delta_x0[2])
        delta_x0[2] = 2 - ca.cos(delta_x0[2])
    ...
    def solve_nlp(self):
        x0_ww = ca.reshape(ca.horzcat(self.sym_x0, self.sym_ww), -1, 1)

        ww_lb = self.w_lb * self.r_horizon
        ww_ub = self.w_ub * self.r_horizon
        lb_x0_ww = [-ca.inf] * 4 + ww_lb
        ub_x0_ww = [ca.inf] * 4 + ww_ub

        nlp = {'f': self.object_function, 'x': x0_ww}

        # init_x0 = self.df[['x', 'y', 'theta', 'v']].iloc[-self.f_horizon-1].to_numpy().reshape(-1)
        init_x0 = self.xk0
        ...
        sol = nlp_solver(x0=ca.DM(np.append(init_x0, init_ww)), lbx=lb_x0_ww, ubx=ub_x0_ww)
        # x_est = np.array(sol['x'][0:4])
        ww_est = np.array(sol['x'][4:]).reshape(self.r_horizon, 4)
        self.df.iloc[self.id_list_mea[:-1], 12:16] = ww_est     # update ww

        # uu = self.df[['U.a_z', 'U.w_y']].iloc[-(self.f_horizon+1):-1].to_numpy().reshape(-1, self.r_horizon)
        # t_stamps = self.df['time'].iloc[-(self.f_horizon+1):].to_numpy()
        # ts = np.diff(t_stamps)

        sym_w_seq = ca.reshape(self.sym_ww, -1, 1)
        f_xo2xn = ca.Function('f', [self.sym_x0, sym_w_seq], [self.sym_xx[:, -1]])

        x_est = np.array(f_xo2xn(sol['x'][0:4], sol['x'][4:])).reshape(4, 1)    # calculate x_N from x_0 and ww
        return x_est
    ...
    def setup_sym_function(self):
        ...
        # y_delta[2] = ca.tan(0.5 * y_delta[2])
        y_delta[2] = 2 - ca.cos(y_delta[2])
