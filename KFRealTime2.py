...
class KFRealTime:
    ...
    def df(self):
        ...
        columns = ['time',  # 0
                   ...
                   'rt_theta',  # 30
                   'rt_v']  # 31
        df = pd.DataFrame(data=[[np.nan] * 32] * n, columns=columns)
        for i in range(n):
            df.iloc[i, 0] = self.dataset['time'][i]
            sensor = self.dataset['sensor'][i]
            df.iloc[i, 1] = sensor
            if sensor == 'imu':
                df.iloc[i, 2:4] = self.dataset['data'][i].reshape(2)
            elif sensor == 'vel':
                df.iloc[i, 4] = self.dataset['data'][i].reshape(1)
            elif sensor == 'tf':
                df.iloc[i, 5:8] = self.dataset['data'][i].reshape(3)
            df.iloc[i, 8:12] = self.dataset['X'][i].reshape(4)
            df.iloc[i, 12:28] = self.dataset['P'][i].reshape(16)
            df.iloc[i, 28:32] = self.dataset['X_rt'][i].reshape(4)
        return df
