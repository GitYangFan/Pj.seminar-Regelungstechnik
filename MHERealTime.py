...
class MHERealTime:
        ...
        if self.initialized:
            if data_typ == 'imu':
                dict_data = {'time':            [t_stamp],
                             ...
                             'U.a_z':           [data[0][0]],
                             'U.w_y':           [data[1][0]]}
                self.update_df_by_dict(dict_data)  # update imu data, states are so far empty
                self.df_all.sort_values(by=['time'], inplace=True)
                i_last = self.df_all.index.to_list().index(len(self.df_all)-1)-1

                ts = t_stamp - self.df_all['time'].iloc[i_last]
                x_last = self.df_all[['x', 'y', 'theta', 'v']].iloc[i_last].to_numpy().reshape(-1, 1)
                u_last =
                ...
                self.df_all.iloc[-1, 16] = t1 - t0
                self.df_all.iloc[-1, 17] = t2 - t1
    ...
    def x2df_all(self, x):
        self.df_all.iloc[-1, 8] = x[0][0]
        self.df_all.iloc[-1, 9] = x[1][0]
        self.df_all.iloc[-1, 10] = (x[2][0] + np.pi) % (2*np.pi) - np.pi        # -pi < theta < pi
        self.df_all.iloc[-1, 11] = x[3][0]
    ...
    def update_obj(self):
        ...
        df_all_horizon = self.df_all.loc[self.id_list_mea[0]:]              # all data with the horizon (len M >= N)
        ...
        self.xk0 = self.df_all[['x', 'y', 'theta', 'v']].iloc[id_x0].to_numpy().reshape(4, 1)
    ...
    def solve_nlp(self):
        ...
        init_xx = self.df_all[['x', 'y', 'theta', 'v']].loc[self.id_list_mea[:-1]].to_numpy()
        init_xx = np.concatenate((init_xx, init_xx[-1, :].reshape(1, 4)), axis=0).reshape(-1)
        # init_ww = np.array([0]*(4*self.r_horizon))
        init_ww = self.df_all[['w_x', 'w_y', 'w_theta', 'w_v']].loc[self.id_list_mea[:-1]].to_numpy().reshape(-1)
        ...
        self.df_all.iloc[self.id_list_mea[:-1], 12:16] = ww_est     # update ww
