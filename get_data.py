...
# Data of TF sensor
# ======================================================================================================================
df_tf = pd.concat([df_raw.iloc[:, 0], df_raw.iloc[:, 33:40]],
                  axis=1)  # take the data from tf only, include global time stamp
...
vel['time'] = data_vel[:, 0].reshape(1, -1)
# time_vel_diff = np.diff(time_vel)
# print('Average time step of velocity data:', np.average(time_vel_diff))

# velocity---------------------------------------
vel['velocity'] = data_vel[:, 1].reshape(1, -1)

print("Data of velocity 'vel': ", list(vel.keys()))
